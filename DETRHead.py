...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        bs, c, h, w = src.shape
        # batch_size, 256, 180, 180 => batch_size, 256, 32400 => 32400, batch_size, 256
        src         = src.flatten(2).permute(2, 0, 1)
        # batch_size, 256, 180, 180 => batch_size, 256, 32400 => 32400, batch_size, 256
        pos_embed   = pos_embed.flatten(2).permute(2, 0, 1)
        # 100, 256 => 100, 1, 256 => 100, batch_size, 256
        query_embed = query_embed.unsqueeze(1).repeat(1, bs, 1)
        # batch_size, 180, 180 => batch_size, 32400
        mask        = mask.flatten(1)
        # 100, batch_size, 256
        tgt         = torch.zeros_like(query_embed)
        
        # 32400, batch_size, 256 => 32400, batch_size, 256
...
class DETRhead(nn.Module):
    def __init__(self, hidden_dim, num_classes, num_queries, aux_loss=False, pretrained=False):
        super().__init__()
    ...
    def forward(self, src):
        pos = torch.ones(1, 1,256,23,23)
        mask = torch.ones(1,23,23)

        # 将主干的结果进行一个映射，然后和查询向量和位置向量传入transformer。
        # batch_size, 512, 180, 180 => batch_size, 256 180, 180 => 6, batch_size, 100, 256
...
model = DETRhead(256,3,100)
model.eval()

# 创建一个示例输入张量
dummy_input = torch.randn(1,1024, 23, 23)
